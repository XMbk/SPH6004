...
def test(X_train, X_test, Y_train, Y_test):
    ...
    for i in range(20):
        ...
        rf = RandomForest(n_classifiers=i*10+1)
        ...
        times.append(t)
        print('accuracy:', acc, 'roc_auc score:', score)
        print("--- Running time: %.6f seconds ---" % (time.time() - start_time))
        scores.append(score)
        n_tree.append(i*10+1)
    ...
    plt.show()
    plt.plot(n_tree, times)
    plt.ylabel('training time')
    plt.xlabel('number of trees')
    plt.savefig('./output/RandomForest_t.jpg')
    plt.show()
